    ...
    QMainWindow(parent)
{
    ...
    CityTableModel *model = new CityTableModel(this);
    model->setInput(m_engine);
...
void MainWindow::setupTreeModel()
{
    ...
    foreach(const QString& country, countries) {
        ...
        QStandardItem *countryItem = new QStandardItem(flag, country); // top-level item

        QStringList cities = m_engine->citiesByCountry(country);
        foreach(const QString& city, cities) {
            QList<QStandardItem*> cityItems;
            cityItems << new QStandardItem(city);
            cityItems << new QStandardItem(QString::number(m_engine->population(city)));
            cityItems << new QStandardItem(QString::number(m_engine->area(city)));
            countryItem->appendRow(cityItems); // append child items to top-level item
        }
